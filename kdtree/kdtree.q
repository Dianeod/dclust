...
tree.createTree:{[sample;diml]
 root:flip `idx`initi`rep`dir`dim`parent`clust`clustIdx`valid!(0;0;enlist @[sample;0];0N;0;0;0;enlist 0;1b); 			/insert first cluster
...
tree.insertKd:{[diml;kd;samp;L;cl] 												/check if its to the left or right of initial cluster in tree
 dirn:{0<=first @[x;0]`idx}{[samp;kd;nn]a:@[nn;0];dir1:$[samp[first a`dim]>(raze a`rep)[first a`dim];1;0];
  i:select from kd where dir=dir1,parent=first a`idx,valid;		                                      			/a is the previous pt, while i is the next pt in the tree to do split on
  (i;a;dir1)}[samp;kd]/(i;i:kd[0]); 												/insert cluster into the tree by looking at splitting dimension of each node
 par:dirn[1];
 dict:`idx`initi`clust`rep`dim`valid`parent`dir!(max[kd`idx]+1;L;L;samp;first diml par[`dim]+1;1b;first par`idx;dirn 2);
 $[0=count nv:where not kd`valid;kd upsert dict;[dict[`clust]:cl;kd[first nv]:dict;kd]]
 } 																/update the info of new node into tree

/Calculating distances between clusters using kd tree
tree.distC:{[kd;pt] 
 distCalc:{[kd;query;cl;bestD]
 newn:nn where{[cl;kd;x](first exec clust from kd where idx=x,valid)<>cl}[cl;kd]each nn:bestD[1]; 				/nodes to search that arent in the same cluster;bd[1]=next pts to search
 ...
 if[(newD[0]<bestD[0])&count[newn]<>0;(bestD[0]:newD[0];bestD[2]:newD[1])]; 							/if new dist is less than current best dist, then that becomes new best dist
 axisD:(raze{[kd;bestD;query;nn]ll:select from kd where idx=nn,valid;dirn:$[(qdim:query[dd])<rdim:(first ll[`rep])
 ...
 (bestD[0];distinct axisD;bestD[2];bestD[3])}; 											/get dists between node and pts based on split dim.if=<than best Dist than 																    /search the leafs.Go up the tree and to the lft/rght depending on position
 dist:{[distCalc;kd;pts]idpts:select parent,clust,rep from kd where idx=pts,valid;{0<>count @[x;1]}distCalc[kd;first idpts`rep;
 first idpts`clust]/(0W;(raze idpts[`parent],raze exec idx from kd where parent=pts,valid)except pts;pts;pts
  )}[distCalc;kd;pt];
 update closDist:dist[0],closIdx:dist[2] from kd where idx=pt 									/update new closDist and idx in tree
 }

/delete from kd tree
tree.deleteN:{[kd;X]
 delN:{kd:x[0];X:x[1];	 													/kdtree,pt to be deleted
  dirn:$[ii:0=count ll:exec idx from kd where dir=1,parent=X,valid;first exec idx from kd where parent=X,dir=0,valid;first ll]; 
  ...
  newP:mindim $[ii;imax;imin]raze({[kd;x]first exec rep from kd where idx=x,valid}[kd]each mindim)[;first exec dim 
   from kd where idx=X,valid]; 													/min/max value of rep pts from mindim based on splitting dim
 ...
 nn:first exec idx from kd where initi=X,valid;
 mm:exec initi from kd where closIdx in nn,valid;
 delCl:{0<>count select from x[0] where parent=x[1],valid}delN/(kd;nn); 		/repeat this until reach a node with no children
 delC:update valid:0b from first delCl where idx=last delCl;
